...
if ( ENABLE_FUNCTIONS_TO_CPL_PAGES === false )
  return;
...
function the_support_material() {
  ...
  if ( $link ) : 
    $link_url    = $link['url'];
    $link_title  = $link['title'] ? $link['title'] : __('Material de apoio', 'startertheme');
...
function the_embed_video($iframe, $date_published, $img) {
  ...
  $html_img = '<img src="'. $img['url'] .'" alt="'. __('Em breve', 'startertheme') .'" class="img-fluid img_coming_soon">';
